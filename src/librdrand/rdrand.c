...
int main() {
    ...
    success = _rdrand32_step(&rand_num); // процессорная инструкция RdRand, генерирующая число при помощи генератора случпйных чисел в процессоре

    // при выполненной генерации, выводим число иначе сообщение об ошибке
    if (success) 
    {
        printf("%u\n", rand_num % 10); // выводим результат генерации ("% 10" задаёт однозначное число)
        return 0;
    }
    else
    {
